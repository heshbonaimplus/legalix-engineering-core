...
# ==========================================
# 2. תקציר מנהלים למתכנן (38 ממצאים: 36 אדום, 2 צהוב, 0 כחול)
# ==========================================
...
set_rtl(p_ex_b)
ex_text = (
    "דוח זה נבנה ככלי עבודה מעשי (Designer Correction & Closure) שמטרתו לקצר לצוות הקונסטרוקציה את זמן העבודה מ-Finding ל-Resolved לקראת שחרור יציקות שלד באתר.\n\n"
    "בבדיקה אותרו 38 ממצאים הנדסיים ממוקדים הדורשים התייחסות מתכנן ועדכון תוכניות:\n"
    "• 36 ממצאים בסיווג RED (חובת תיקון ועדכון שרטוט/חישוב טרם ביצוע יציקות).\n"
    "• 2 ממצאים בסיווג YELLOW (נדרשת בדיקה / הכרעת מתכנן / תיאום מול אדריכלות).\n"
    "• 0 ממצאים בסיווג BLUE.\n\n"
    "עבור כל אחד מ-38 הממצאים, הדוח מציג מבנה עבודה תלת-אזורי ממוקד:\n"
...
prio_data = [
    ('🔥 P1 — עדכון שרטוטי זיון ומודל', 'הוספת חישוקי חדירה, עיבוי זיון עליון/תחתון, שינוי קוטר מוטות ודירוג חפיפות', '15–20 דקות לממצא', '20 ממצאים'),
    ('🤝 P2 — דורש תיאום יועצים (MEP/AR)', 'שרוולי מעבר בקורות, פתחי פירים סמוך לעמודים, גובה ראש וסופרפוזיציה', 'דורש תיאום מול אדריכלות ו-MEP', '8 ממצאים'),
    ('📐 P3 — דורש כיול חישוב ומודל ETABS', 'קירות טרנספר, גזירת בסיס סיסמית (R=4.5), שקיעות דיפרנציאליות ועומס כבאית', '1–2 שעות (הרצת מודל / חישוב)', '10 ממצאים'),
    ('💡 P4 — אופטימיזציה שלד וזיון', 'בחינת מעבר למצמדים מכניים (Couplers) ושיפור צפיפות זיון', 'הנדסת ערך (לשיקול יזם/מתכנן)', '—')
]
...
dash_data = [
    ('🔴 RED', 'נדרש תיקון — חריגה מקריטריון תכן מחייב, תת-זיון בכפיפה/גזירה, כשל חדירה או יציבות', 'חובת תיקון בתוכניות ובמודל טרם יציקה', '36'),
    ('🟡 YELLOW', 'נדרשת בדיקה / החלטת מתכנן / תיאום — פער הדורש הכרעה הנדסית או תיאום אדריכלי', 'בחינת חלופות וקבלת החלטת מתכנן', '2'),
    ('🔵 BLUE', 'המלצת אופטימיזציה / הנדסת ערך — המלצה לשיפור ביצועיות, הפחתת צפיפות ברזל וחיסכון', 'לשיקול דעת המתכנן והיזם', '0'),
...
# 38 Structural Finding Cards
from gen_full_45_st_cards import get_full_45_structural_cards
structural_cards = get_full_45_structural_cards()

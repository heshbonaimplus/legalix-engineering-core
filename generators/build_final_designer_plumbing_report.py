...
setup_clean_header_footer(section1, "פרויקט לוד ניר צבי | אינסטלציה, ספרינקלרים וכיבוי אש — Legalix Designer Correction & Closure")
...
set_rtl(p_sub)
r_sub = p_sub.add_run('מערכות אינסטלציה סניטרית, ספרינקלרים וכיבוי אש — תוכנית עבודה וסגירת ממצאים ל-Rev 02')
...
meta_str = (
    ...
    "גרסת מודל שנבדקה: LOD_PL_339_R23 / Lod-FP-PR-R25\n"
...
ex_text = (
    ...
    "עבור כל אחד מ-30 הממצאים שאותרו במודל, הדוח מציג מבנה עבודה תלת-אזורי ממוקד:\n"
...
prio_data = [
    ('🔥 P1 — תיקון ישיר במודל', 'הזזת ספרינקלרים מקורות, שיפועי ניקוז 1.5%, הוספת שסתומי אל-חוזר וראשי Below-Duct', '10–15 דקות לממצא', '16 ממצאים'),
    ('🤝 P2 — דורש תיאום יועצים', 'שרוולים בשלד, קיר מאגרים אטום W8 מוסף קריסטלי, לולאת סניקה והסטת צנרת ממ״ד', 'דורש תיאום חיצוני (שלד/אדריכלות)', '8 ממצאים'),
    ('📐 P3 — דורש חישוב הידראולי', '3 אזורי לחץ למניעת 9.9 בר, עולי סניקה Ø6\", וספיקת צמ״גי גגות', '30–45 דקות (חישוב הידראולי)', '5 ממצאים'),
    ('💡 P4 — המלצת אופטימיזציה', 'מעבר לראשי ספרינקלר K=8.0 בחניון ושימוש בצנרת CPVC כתומה בדירות', 'הנדסת ערך (לשיקול יזם/מתכנן)', '1 ממצא')
]
...
# Read detailed 30 cards from previous data
from build_designer_action_closure_report import plumbing_designer_cards

# Enhance with Estimated Effort
effort_map = {
    "P1": "10–15 דקות (תיקון ישיר במודל רוויט)",
    "P2": "תיאום יועצים (שלד / אדריכלות / חשמל)",
    "P3": "30–45 דקות (כיול חישוב הידראולי / ספיקות)",
...
setup_clean_header_footer(section2, "פרויקט לוד ניר צבי | אינסטלציה — תוכנית פעולה וסגירת ממצאים ל-Rev 02")
...
setup_clean_header_footer(section3, "פרויקט לוד ניר צבי | אינסטלציה — הנחיות להגשת Rev 02")
...
doc.save(doc_path)
print("Complete Designer-Facing Action & Closure Report for Plumbing generated successfully at:", doc_path)

...
doc_path = '/home/yogi/lod_project/ספריית_חוקי_תכן_הנדסית_מאסטר_HVAC.docx'
doc = docx.Document()
...
set_rtl(p_title)
r_title = p_title.add_run('ספריית חוקי תכן הנדסית מאסטר — מנוע בקרת תכן MEP דיגיטלי (HVAC Rule Library)')
...
set_rtl(p_sub)
r_sub = p_sub.add_run('פרויקט: לוד ניר צבי (עמרם אברהם) | ארכיטקטורת 3 מנועים | מודל 4 סטטוסים (אדום / צהוב / כחול / ירוק)')
...
set_rtl(p_desc)
r_desc = p_desc.add_run('1. מנוע גיאומטריה (Geometry Engine): מודד בדיוק מילימטרי התנגשויות, מרחקים, גבהי נטו FFL, שיפועים, שטחי חתך וקטרים ישירות ממודלי RVT / IFC / DWG.\n2. מנוע כללים הנדסיים (Deterministic Rules Engine): משווה את המדידות מול ספי תכן ותקנים ישראליים ובינלאומיים במספרים מוחלטים (לדוגמה: IF Air_Velocity > 12.0 m/s -> RED).\n3. שכבת בינה מלאכותית (AI Synthesizer Layer): אינה ממציאה נתונים, אלא מסבירה את משמעות הכשל, מחשבת את פער החריגה, ומייצרת הצעת תיקון כירורגית לאישור היועץ.\n4. מודל Digital Twin & BCF: הצגת הממצאים ישירות על גבי המודל התלת-ממדי ברוויט עם אפשרות הכרעה ליועץ (אישור / דחייה / אימוץ המלצה).')
...
status_data = [
    ('🔴 אדום (RED)', 'חובה לתקן — כשל קריטי, בטיחות חיים, התנגשות קשה בשלד או פסילת תקן מחייבת (Life Safety & Hard Clash).', 'חובת תיקון מיידי בתוכניות ובמודל ⛔'),
    ('🟡 צהוב (YELLOW)', 'דורש החלטת יועץ / תיאום ביצוע מול קונסטרוקציה, אדריכלות או חלופת שטח מקובלת (Engineering Coordination).', 'קבלת החלטת יועץ והגדרת מפרט ביצוע ⚠️'),
    ('🔵 כחול (BLUE)', 'המלצת אופטימיזציה, הנדסת ערך (Value Engineering), יעילות אנרגטית וחיסכון בעלויות ליזם (Efficiency).', 'המלצה כלכלית לבחירת היזם והמתכנן 💡'),
    ('🟢 ירוק (GREEN)', 'נבדק במודל ואומת כתקין ב-100% לפי כל דרישות התקנים והחישובים ההנדסיים (Verified Compliant Pass).', 'נבדק ואושר לתעודת גמר / טופס 4 ✅')
]
...
json_path = '/home/yogi/lod_project/HVAC_RULE_LIBRARY.json'
...
set_rtl(p_rhead)
r_rhead = p_rhead.add_run('ספריית חוקי התכן המובנית (Core Engineering Rules Schema):')
...
doc.save(doc_path)
print('Master Rule Library DOCX generated successfully at:', doc_path)

...
doc_path = '/home/yogi/lod_project/ספריית_חוקי_תכן_הנדסית_מאסטר_חשמל_ומתח_נמוך.docx'
doc = docx.Document()
...
set_rtl(p_title)
r_title = p_title.add_run('ספריית חוקי תכן הנדסית מאסטר — חשמל, מערכות חירום ומתח נמוך (Electrical Rule Library)')
...
set_rtl(p_desc)
r_desc = p_desc.add_run('1. מנוע גיאומטריה (Geometry Engine): מודד בדיוק מילימטרי תוואי סולמות כבלים, מרווחי אוויר מתעלות מיזוג, גבהי שקעים ומפסקים, ומיקומי גופי תאורת חירום ישירות ממודלי RVT / IFC / DWG.\n2. מנוע כללים הנדסיים (Deterministic Rules Engine): משווה את המדידות מול ספי תכן, חוק החשמל, תקנות חח״י, ת״י 1220, ת״י 1430 ו-NFPA 70/110 במספרים מוחלטים (לדוגמה: IF LifeSafetyCircuit.HasRCD == True -> RED).\n3. שכבת בינה מלאכותית (AI Synthesizer Layer): מנתחת את עומסי הקצר, מפלי המתח, בטיחות החיים והגנת האש, ומייצרת הצעת תיקון כירורגית ליועץ.\n4. מודל Digital Twin & BCF: הצגת הממצאים ישירות על גבי המודל התלת-ממדי ברוויט עם אפשרות הכרעה ליועץ (אישור / דחייה / אימוץ המלצה).')
...
status_data = [
    ('🔴 אדום (RED)', 'חובה לתקן — סכנת התחשמלות, שריפה מחשמל, כשל בהזנת משאבות כיבוי אש ומפוחי עשן, קצר מתח גבוה ופסילות חח״י/כב״ה.', 'חובת תיקון מיידי בתוכניות ובמודל ⛔'),
    ('🟡 צהוב (YELLOW)', 'דורש החלטת יועץ / תיאום ביצוע מול קונסטרוקציה, אדריכלות או מפרט תיוג ואיזון פאזות בלוחות.', 'קבלת החלטת יועץ והגדרת מפרט ביצוע ⚠️'),
    ('🔵 כחול (BLUE)', 'המלצת אופטימיזציה, הנדסת ערך (Value Engineering), ניהול עומסי רכב חשמלי (DLM) וחיסכון בעלויות שנאים ליזם.', 'המלצה כלכלית לבחירת היזם והמתכנן 💡'),
    ('🟢 ירוק (GREEN)', 'נבדק במודל ואומת כתקין ב-100% לפי כל דרישות חוק החשמל ותקנות חח״י/כב״ה.', 'נבדק ואושר לתעודת גמר / טופס 4 ✅')
]
...
set_rtl(p_rhead)
r_rhead = p_rhead.add_run('ספריית חוקי התכן המובנית לחשמל ומתח נמוך (Electrical & LV Rules Schema):')
...
doc.save(doc_path)
print('Master Electrical Rule Library DOCX generated successfully at:', doc_path)

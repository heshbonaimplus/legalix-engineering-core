...
# 2. תקציר מנהלים למתכנן ולשיווק (David 12)
...
# 4. טבלת איפיון מכר מול ביצוע (David 12)
...
from build_marketing_spec import chapters_spec_mkt
...
for ch_title, sections in chapters_spec_mkt:
    ...
    for r_i, (s_num, s_desc, s_tier) in enumerate(sections, start=1):
        row = t_sec.rows[r_i]
        for col_idx, text in enumerate([s_num, s_desc, "כן (נבדק בסבב 1 + 2) ✅"]):
            cell = row.cells[col_idx]
